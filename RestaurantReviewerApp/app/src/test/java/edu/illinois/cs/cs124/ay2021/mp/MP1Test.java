...
public final class MP1Test {
  ...
  public static class UnitTests {
    ...
    public void testLoadRestaurantFields() throws IOException {
      ...
      assertWithMessage("Wrong restaurant count")
          .that(restaurantList)
    ...
    public void testRestaurantListView() {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        onView(withRecyclerView(R.id.recycler_view).atPosition(position))
            .check(matches(hasDescendant(containsText(sortedRestaurants.get(position).getName()))));
        onView(withRecyclerView(R.id.recycler_view).atPosition(position))
            .check(
                matches(hasDescendant(containsText(sortedRestaurants.get(position).getCuisine()))));
      }
    }
